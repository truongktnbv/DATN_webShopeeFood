        ...
        @foreach($products as $product)
            <div class="item">
                ...
                <div class="item__info">
                    <a href="#" title="Đăng nhập" class="cate">{{ $product->category->c_name ?? "[N\A]" }}</a>
                    @if ($product->pro_sale)
                        <a href="" title="SaleOff" class="cate sale">-{{ $product->pro_sale }}%</a>
                    @endif
                    <a href="{{ route('get.product.detail',$product->pro_slug . '-'.$product->id ) }}" title="Đăng nhập DD6014B" class="name">{{  $product->pro_name }}</a>
                    @if ($product->pro_sale) 
                        <p class="price">
                            <span>Giá bán: </span>
